...
const queryChain = (done) => {
  ...
  const foundPeople = Person.find({ favoriteFoods: foodToSearch });
  const filteredPeople = foundPeople.sort('name').limit(2).select('name favoriteFoods');
  filteredPeople.exec((err, data) => {
    if (err) return console.error(err);
    console.log(data)
    done(null, data);
  })
